        ...
        <CardSwipe
          elements={cards.map((card) => (
            <Tooltip>
              ...
              <TooltipContent className="z-1000" side="top" align="center">
                <p className="font-paragraph bg-primary">
                  Click {card.name} for more details
                </p>
              </TooltipContent>
            </Tooltip>
          ))}
...
const BankAccountCardsCard = ({
  account,
  cards,
  className,
  ...props
}: BankAccountCardsProps) => {
  ...
  const cardsList = () => {
    if (cards.length === 0) {
      ...
      return CreditCardSwipe(setDialogOpen, account, cards, handleCardClick);
  };
  ...
  return (
    <>
      <Card className={cn("border-0 content-center", className)} {...props}>
        ...
        <CardContent>{cardsList()}</CardContent>
      </Card>
